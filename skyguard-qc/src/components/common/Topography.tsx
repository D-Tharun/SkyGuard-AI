...
interface WebGLContextData {
  renderer: Renderer;
  program: Program;
  mesh: Mesh;
}

const ctxMap = new WeakMap<HTMLElement, WebGLContextData>();
...
export const Topography: React.FC<TopographyProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Graceful fallback if WebGL 2 cannot be initialized
      return;
    }
    ...
    // Accessibility: check reduced motion preference ONLY if user explicitly enabled it
    ...
    const renderFrame = (t: number) => {
      const time = prefersReducedMotion ? 1.0 : (t - t0) * 0.001;
      const u = program.uniforms;
      u.iTime.value = time;
      const ma = u.uMorphAmount.value as number;
      const sp = prefersReducedMotion ? 0 : (u.uSpeed.value as number);
      const msp = prefersReducedMotion ? 0 : (u.uMorphSpeed.value as number);
    ...
    return () => {
      ...
      try {
        ...
        // Container might already be unmounted
      }
