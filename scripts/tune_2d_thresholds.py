import os
import sys
import pandas as pd
import numpy as np

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(BASE_DIR)

from src.visualization.evaluate_models import load_data, run_base_models
from src.qc.fusion import EventAwareFusionEngine

def evaluate_thresholds(df_base, ml_gate, spatial_thresh):
    class TunedFusionEngine(EventAwareFusionEngine):
        def determine_final_state(self, row):
            S_a = row['S_anomaly']
            S_e = row['S_event']
            spatial_avail = row['spatial_evidence_available']
            confidence = 0.90 if spatial_avail else 0.70
            
            if S_a < 0.5 and S_e < 0.5:
                return "NORMAL", "Low anomaly, low event evidence.", confidence
                
            if S_a < 0.5 and S_e >= 0.5:
                return "GENUINE_EXTREME_EVENT", "Low fault evidence, high event evidence.", confidence
                
            if S_a >= 0.5 and S_e < 0.5:
                fault_type = "SENSOR_FAULT"
                if row.get('spike_flag', 0) > 0: fault_type = "SPIKE_FAULT"
                elif row.get('freeze_flag', 0) > 0: fault_type = "FROZEN_FAULT"
                elif row.get('lstm_score', 0) > 0.6: fault_type = "DRIFT_FAULT"
                elif row.get('if_score', 0) > 0.6: fault_type = "MULTIVARIATE_FAULT"
                
                if row.get('temperature_c', 0) == 9999.0 or row.get('relative_humidity_pct', 0) == -999.0 or row.get('temperature_c', 0) > 65:
                    fault_type = "RANGE_FAULT"
                if pd.isna(row.get('temperature_c')) or pd.isna(row.get('relative_humidity_pct')) or pd.isna(row.get('surface_pressure_hpa')):
                    fault_type = "COMMUNICATION_FAULT"
                    
                return fault_type, "High fault evidence, low event evidence.", confidence
                
            if S_a >= 0.5 and S_e >= 0.5:
                if spatial_avail:
                    current_thresh = 1.0
                    if row.get('lstm_score', 0) > ml_gate or row.get('if_score', 0) > ml_gate:
                        current_thresh = spatial_thresh
                        
                    if row.get('spatial_deviation_max_norm', 0) > current_thresh:
                        return "SENSOR_FAULT", f"Deviation > {current_thresh}", confidence
                    else:
                        return "GENUINE_EXTREME_EVENT", "Coherent", confidence
                else:
                    return "UNCERTAIN_REVIEW", "Conflict without spatial.", confidence
                    
            return "NORMAL", "Fallback", 0.0

    fusion = TunedFusionEngine()
    df_full = fusion.run_fusion(df_base)
    
    genuine_mask = (df_full['is_genuine_event'] == 1) & (df_full['fault_type'] == 'NONE')
    n_obs = genuine_mask.sum()
    
    preds = ~df_full['final_state'].isin(['NORMAL', 'GENUINE_EXTREME_EVENT', 'UNCERTAIN_REVIEW'])
    
    false_alerts = preds[genuine_mask].sum()
    fpr = false_alerts / n_obs if n_obs > 0 else 0
    gepr = 1 - fpr
    
    event_fault_mask = (df_full['is_genuine_event'] == 1) & (df_full['fault_type'] != 'NONE')
    total_faults = event_fault_mask.sum()
    detected = preds[event_fault_mask].sum()
    recall = detected / total_faults if total_faults > 0 else 0
    
    return gepr, recall

def main():
    print("=========================================")
    print(" 2D DYNAMIC THRESHOLD GRID SEARCH        ")
    print("=========================================\n")
    
    df_val = load_data('validation_benchmark_injected.csv')
    df_val = run_base_models(df_val, is_demo=True, use_real_ml=False) # Use mock signals for testing decision logic
    
    ml_gates = [0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]
    spatial_thresholds = [0.00, 0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50, 0.70, 1.00]
    
    results = []
    
    print(f"{'ML Gate':<8} | {'Spatial Thresh':<14} | {'GEPR':<10} | {'Fault Recall':<15}")
    print("-" * 60)
    
    for ml in ml_gates:
        for st in spatial_thresholds:
            gepr, recall = evaluate_thresholds(df_val, ml, st)
            results.append((ml, st, gepr, recall))
            print(f"{ml:<8.2f} | {st:<14.2f} | {gepr:<10.4f} | {recall:<15.4f}")
            
    # Selection Rule: First require GEPR >= 0.99, Then maximize fault recall during genuine events.
    valid_candidates = [r for r in results if r[2] >= 0.99]
    if not valid_candidates:
        print("ERROR: No candidate found with GEPR >= 0.99")
        return
        
    best_candidate = max(valid_candidates, key=lambda x: (x[3], x[1])) # max recall, then break ties with higher spatial threshold for robustness
    
    print("\n=========================================")
    print(" SELECTED PARAMETERS ON VALIDATION SET   ")
    print("=========================================")
    print(f"ML Confidence Gate: {best_candidate[0]:.2f}")
    print(f"Dynamic Spatial Threshold: {best_candidate[1]:.2f}")
    print(f"Validation GEPR: {best_candidate[2]:.4f}")
    print(f"Validation Fault Recall: {best_candidate[3]:.4f}")
    
    with open(os.path.join(BASE_DIR, 'scripts', 'selected_params.txt'), 'w') as f:
        f.write(f"{best_candidate[0]:.2f}\n{best_candidate[1]:.2f}")

if __name__ == "__main__":
    main()
